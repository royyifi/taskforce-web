...
export async function POST(request: Request) {
  ...
  let finalLogoFileId: string | null = partner.logoFileId;
  if (!finalLogoFileId && proposalLogoFileId) {
    ...
    finalLogoFileId = null;
  } else if (finalLogoFileId) {
    const logo = await db.storedFile.findUnique({ where: { id: finalLogoFileId } });
  ...
  const activity = await db.activity.create({ data: {
    ...
    dosenName: lecturerRows[0]?.name || null, rkpStatus: rkpUrl ? "DIAJUKAN" : "BELUM_ADA", rkpUrl: rkpUrl || null, spmUrl: spmUrl || null, iaPartnerLogoFileId: partner.logoFileId, iaProposalLogoFileId: proposalLogoFileId ? String(proposalLogoFileId) : null, submitterNim, submitterPhone: submitterPhone || null, submitterUnit: prodi || submitterUnit || null,
...
export async function PATCH(request: Request) {
  ...
  if (status === "APPROVED" && submission.iaProposalLogoFileId) {
    const proposalLogo = await db.storedFile.findUnique({ where: { id: submission.iaProposalLogoFileId } });
    if (proposalLogo) {
      const oldLogoId = submission.partner.logoFileId;
      await db.$transaction([
        db.partner.update({ where: { id: submission.partnerId }, data: { logoFileId: submission.iaProposalLogoFileId } }),
        db.activity.update({ where: { id }, data: { iaPartnerLogoFileId: submission.iaProposalLogoFileId, iaProposalLogoFileId: null, status, reviewNote: reviewNote || null, reviewedAt: new Date(), iaStatus: "DIAJUKAN", iaSubmittedAt: new Date(), iaReviewNote: null } }),
      ]);
      if (oldLogoId && oldLogoId !== submission.iaProposalLogoFileId) await db.storedFile.deleteMany({ where: { id: oldLogoId } });
      await logAudit({ action: "UPDATE", entityType: "Partner", entityId: submission.partnerId, entityName: submission.partner.name, detail: `Logo mitra diperbarui dari pengajuan oleh admin.` });
    }
  } else {
